...
def gen_combo(s):
    """generates combination for swap gate insertion"""
    for i in range(s + 1):
        s2 = s - i
        yield (i, s2)
...
def interaction_frequency_ckt_window_ln(qubit, line):
    if(line==len(lines)):
        lookahead_window=0
    else:
        lookahead_window=math.ceil(math.log(len(lines)-line))
        print("cw: ", lookahead_window)
    interactions = l[int(qubit)-1]
    count=0
    tot_int=0
    window_count=0
    for i in interactions:
        if(i<=line):
            count+=1
        elif(i>line and window_count<=lookahead_window):
            window_count+=1
            tot_int+=i
...
def interaction_factor(qubit, line):
    interactions = l[int(qubit)-1]
    count=0
    tot_int=0
    for i in interactions:
...
##    print(count)
    try:
        remaining_gates=len(lines) - count
        lookahead_factor = tot_int/remaining_gates
    except ZeroDivisionError:
        lookahead_factor=tot_int
    except IndexError:
        lookahead_factor=1
...
def rearrange_grid(grid, q, t, line):
    ...
    for path in minPath:
        path_int_freq=0
        for qubit in path:
            path_int_freq += interaction_factor(re.findall('\d+', grid[qubit[0]][qubit[1]])[0], line)
